...
<body>
	<%
		...
		// 进行查询
		List<LibraryBean> libraryList = queryRunner.query(librarySql, new BeanListHandler<>(LibraryBean.class));
		List<Book_SortBean> bookSortList = queryRunner.query(bookSortSql, new BeanListHandler<>(Book_SortBean.class));
		...
		for (int i = 0; i < libraryList.size(); i++) {
			libraryJson.put(libraryList.get(i).getId(),libraryList.get(i).getName());
		}
		// 获取分类json
		for (int i = 0; i < bookSortList.size(); i++){
			bookSortJson.put(bookSortList.get(i).getId() , bookSortList.get(i).getName());
		}
%>
